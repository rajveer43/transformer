...
class Seq2SeqTransformer(nn.Module):
    ...
    def forward(self,
        ...
        outs = self.transformer(src_emb, tgt_emb, src_mask, tgt_mask, None, # None: memory_mask. Not needed
                                # since its not self-attention. Only padding mask is needed from src.
...
def create_mask(src, tgt):
    ...
    # tgt_mask upper right triangle to prohibit that words cannot benefit from following words which should be decoded.
    # as uni directional rnn from left to right no information flow from right to left.
    # src_mask no mask like bidirectional rnn

    # padding needed to be able to work with variable sequence length input and output with tensors.
    src_padding_mask = (src == PAD_IDX).transpose(0, 1)
    # if padding added in src it shouldn't be regarded as info.

    tgt_padding_mask = (tgt == PAD_IDX).transpose(0, 1)
    # if padding, we will not consider it in optimization.
...
# print('Source vocabulary size {}'.format(SRC_VOCAB_SIZE))
# print('Target vocabulary size {}'.format(TGT_VOCAB_SIZE))
...
print(translate(transformer, "Je suis froid ."))
